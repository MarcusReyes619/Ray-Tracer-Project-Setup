...
{
    glm::vec3 target = raycastHit.point + raycastHit.normal + ray::randomInUnitSphere();
...
bool Metal::Scatter(const ray_t& ray, const raycastHit_t& raycastHit, glm::vec3& color, ray_t& scattered) const
{
    ...
    glm::vec3 reflected = ray::reflect(glm::normalize(ray.m_direction), raycastHit.normal);
    ...
    scattered = ray_t{ raycastHit.point, reflected + (ray::randomInUnitSphere() * m_fuzz) };
    color = m_albedo;
...
bool Dielectric::Scatter(const ray_t& ray, const raycastHit_t& raycastHit, glm::vec3& color, ray_t& scattered) const
{
    ...
    if (ray::refract(ray.m_direction, outNormal, ni_over_nt, refracted))
    {
        // use schlick and cosine (angle) to determine if ray is reflected or refracted
        reflectProbability = ray::schlick(cosine, m_index);
    }

    // create reflected or refracted ray
    scattered = (ray::random01() < reflectProbability) ? ray_t{ raycastHit.point, reflected } : ray_t{ raycastHit.point, refracted };
    color = m_albedo;
